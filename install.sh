...
v2ray_install() {
  ...
  mkdir -p /root/v2ray
  cd /root/v2ray || exit
  wget -N --no-check-certificate https://raw.githubusercontent.com/chinayin/v2ray_install/${github_branch}/v2ray.sh
...
acme_register_account(){
  if "$HOME"/.acme.sh/acme.sh --register-account -m "${random_num}@acme.com" --server zerossl; then
    ...
    echo -e "${Error} ${RedBG} 注册ZeroSSL账号失败 ${Font}"
    exit 1
  fi
}
acme() {
...
v2ray_conf_add_tls() {
  cd /etc/v2ray || exit
  wget --no-check-certificate https://raw.githubusercontent.com/chinayin/v2ray_install/${github_branch}/tls/config.json -O config.json
  modify_path
  modify_alterid
  modify_inbound_port
  modify_UUID
...
v2ray_conf_add_h2() {
  cd /etc/v2ray || exit
  wget --no-check-certificate https://raw.githubusercontent.com/chinayin/v2ray_install/${github_branch}/http2/config.json -O config.json
  modify_path
  modify_alterid
  modify_inbound_port
  modify_UUID
...
update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/chinayin/v2ray_install/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
...
menu() {
  ...
  echo -e "\thttps://github.com/chinayin/v2ray_install\n"
  ...
  2)
    ...
    ;;
  3)
    bash <(curl -L -s https://raw.githubusercontent.com/chinayin/v2ray_install/${github_branch}/v2ray.sh)
    ;;
  4)
